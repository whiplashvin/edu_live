...
function Chat() {
  ...
  return (
    <div className="bg-zinc-200 row-span-4 rounded-md p-2 h-full flex flex-col justify-between relative">
      {isFetching && (
        <p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-300 font-extralight text-sm">
          fetching...
        </p>
      )}
      <section
      ...
      >
        {displayText.map((c, index) => (
          ...
          >
            <span
              className={`${c.user === CurrUser ? "hidden" : "flex"} size-3 md:size-6 p-2 text-[8px] md:text-xs font-extralight rounded-full bg-zinc-900  justify-center items-center text-indigo-300`}
            >
      ...
      <div className="w-full p-2 flex items-center gap-2">
        ...
        <IoSendSharp
          className="text-sm md:text-xl text-zinc-600 cursor-pointer hover:scale-110 hover:text-indigo-300"
          onClick={() => {
            if (message.length > 0) {
              send();
            }
