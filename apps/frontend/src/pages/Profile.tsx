...
function Profile() {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-sm md:text-2xl text-indigo-600 font-extralight p-4 pl-8 my-5">
        Profile
      </h1>
      <div className="w-[95%] mx-auto flex flex-col items-start px-5 py-10 lg:flex-row bg-indigo-100/70 rounded-xl gap-10 relative mt-10 shadow-lg border border-zinc-200">
        <div className="size-20 rounded-full bg-white flex items-center justify-center">
          <CiUser fontSize={40} color="#343a40" />
        </div>
        <div className="flex flex-col gap-3">
          <p className="text-lg font-extralight text-indigo-500">{User}</p>
        ...
        <div className="flex justify-center w-full lg:w-fit lg:absolute lg:top-5 right-5">
          <button
            className="bg-indigo-400/30 border-[0.5px] border-indigo-500 text-zinc-950 px-4 py-2 rounded-lg flex items-center text-sm font-thin hover:bg-indigo-600/30 gap-2 shadow-lg"
            disabled={true}
          >
            Update Profile
          </button>
